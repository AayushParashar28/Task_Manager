...
exports.postTask = async (req, res) => {
  try {
    ...
    const task = await Task.create({ user: req.user.id, title, description, status, dueDate , priority }); // Store title and status
...
exports.putTask = async (req, res) => {
  try {
    ...
    if (!title && !description && !status) {
      return res.status(400).json({ status: false, msg: "At least one field (title, description, status) must be provided" });
    }
